...
Heat waves are now one of the biggest threats worldwide to our environment and public health, particularly in regions where summers can be extremely hot. Those at greatest risk of dehydration, heat exhaustion and even death from heat stroke as a result of working outdoors for long periods of time in extreme heat include construction workers, farmers, delivery workers, traffic police officers and those that are employed in industrial jobs.

Most currently available wearable health monitoring technologies focus on providing the user with information related to their physiological state, but lack the ability to physically protect them from heat related stresses. Portable cooling technologies are also unable to provide any type of intelligent health monitoring or analysis of their user.

This project will therefore explore the development of a Smart IoT-based Heat Wave Protection System which will include two interconnected wearable devices – a Smart Health Monitoring Band and an Intelligent Neck Cooling Device. The Smart Health Monitoring Band will consist of multiple sensor-based devices that will continuously monitor the user's body temperature, heart rate, blood oxygen, physical activity level, and ambient weather conditions, transmitting this information wirelessly (using BLE and Wi-Fi) for real time analysis of the user's health and their environment.

The analysis of both the users physiological and the environment conditions will allow for the detection of heat stress, as well as risk of heat stroke. If either is detected, the intelligent neck cooling device will automatically activate the thermoelectric cooling system (based on a TEC1-12706 Peltier device) built into it, to cool the user's neck.
